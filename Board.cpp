...
if(player_number!=this->board[source.first][source.second]->getPID() || this->board[source.first][source.second]==nullptr ||  board.empty()){
    ...
    for (size_t i = 0; i < this->board.size(); i++){
      for (size_t j = 0; j < this->board.front().size(); j++){
          if(this->board[i][j]!=nullptr && this->board[i][j]->getPID()==player_number){
           //  cout<<this->board[i][j]->cur_Hp<<endl;
           //  cout<<i<<" "<<j<<endl;
            return true;
          }
      }
